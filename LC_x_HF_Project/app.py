...
st.subheader("Summarize text content from a YouTube video or a website URL using LangChain and HuggingFace models.")

repo_id = "deepseek-ai/DeepSeek-V3.1"

with st.sidebar:
    st.header("Input Options")
    hf_ai_api_key = st.text_input("Enter your HuggingFace API Key", type="password")
    if hf_ai_api_key and st.button("Initialize LLM"):
    ...
    elif not hf_ai_api_key and st.button("Initialize LLM"):
        st.session_state.chat_model = None
        st.error("Please enter your HuggingFace API Key to initialize the LLM.")
...
if st.button("Summarize Context"):
    if not hf_ai_api_key.strip() or not url.strip():
        st.error("Please provide both the HuggingFace API Key and a valid URL.")
